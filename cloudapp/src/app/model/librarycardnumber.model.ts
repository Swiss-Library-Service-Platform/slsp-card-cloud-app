...
export class Librarycardnumber {
  static allowedRegex = [
    ...
    /^290\d{6,7}$/i,
    // IDS Basel
    /^[AB][A-Z\d]*\d{2,}[A-Z\d]*$/i,
    /^(A[A-Z\d]*\d{2,}[A-Z\d]*|MBS[A-Z\d]*\d{2,}[A-Z\d]*|\d{7}|\d{11})$/i,
    // IDS Bern
    /^B[A-Z\d]*\d{2,}[A-Z\d]*$/i,
    /^(B[A-Z\d]*\d{2,}[A-Z\d]*|\d{9})$/i,
    // IDS Luzern
    /^L\-[A-Z\d]*\d{2,}[A-Z\d]*$/i,
    /^(L[A-Z\d]*\d{2,}[A-Z\d]*|FHZ\d{7})$/i,
    // IDS St. Gallen
    /^H\-[A-Z\d]*\d{2,}[A-Z\d]*$/i,
    /^(H[A-Z\d]*\d{2,}[A-Z\d]*|\d{13})$/i,
    // IDS Zürich Universität
    /^U\-[A-Z\d]*\d{2,}[A-Z\d]*$/i,
    /^([UM][A-Z\d]*\d{2,}[A-Z\d]*)$/i,
    // NEBIS
    // This regix unfortunately matches a lot of numbers
    /^E\-[A-Z\d]*\d{2,}[A-Z\d]*$/i,
    /^([ESP][A-Z\d]*\d{2,}[A-Z\d]*)$/i,
    // IDS Zürich Zentralbibliothek
    /^(Z\-[A-Z\d]*\d{2,}[A-Z\d]*|E\-[A-Z\d]*\d{2,}[A-Z\d]*)$/i,
    /^[ZY][A-Z\d]*\d{2,}[A-Z\d]*$/i,
    ...
    /^G\-[A-Z\d]*\d{2,}[A-Z\d]*$/i,
    /^(G\d+|GKBG\d+|GKSB\d+|GKSH\d+|GKWA\d+|GKWI\d+|GBAR\d+|GBEB\d+|GKSS\d+)$/i,
    // Sistema Bibliotecario Ticinese (SBT)
    /^T\-[A-Z\d]*\d{2,}[A-Z\d]*$/i,
    /^(T[A-Z\d]*\d{2,}[A-Z\d]*|202\d{7})$/i,
  ...
  static isRemovable(libraryCardNumber: object): boolean {
    ...
    return libraryCardNumber['id_type']['value'] == '02' && matchNote == null;
  }
  ...
  static isDashedLibraryCardNumber(libraryCardNumber: object): boolean {
    const matchImma = libraryCardNumber['value'].match(
      /(\d{2})-(\d{3})-(\d{3})/,
    );
   ...
   * @memberof Librarycardnumber
   */
  static isValidImmatriculationNumber(
    immatriculationNumber: string,
    useStrictChecking = true,
  ) {
