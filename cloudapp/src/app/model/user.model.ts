...
export class User {
   ...
   * @memberof User
   */
  getUserBlocks(blockDescriptionValue = ''): Map<string, any> {
    const blocks = new Map<string, any>();

    if (!this.userValue['user_block'] || this.userValue['user_blok'])
      return blocks;

    for (const userblock of this.userValue['user_block']) {
      // Block is not active
      if (userblock['block_status'].toUpperCase() !== 'ACTIVE') continue;

      // Block expired
      if (userblock['expiry_date']) {
        const expireDate = new Date(userblock['expiry_date']);
      ...
      if (
        ...
        userblock['block_description']['value'] !== blockDescriptionValue
      )
        continue;
      // Add Block
  ...
  removeBlock(blockType: string): boolean {
    const initialCount = this.userValue['user_block'].length;

    this.userValue['user_block'] = this.userValue['user_block'].filter(
      function (userblock) {
  ...
  getAddresses(): Array<object> {
    ...
    const addresses = JSON.parse(
      JSON.stringify(this.userValue['contact_info']['address']),
    );
  ...
  getLibraryCardNumbers(): Array<string> {
    const libraryCardNumbers = [];

    if (
      !Array.isArray(this.userValue['user_identifier']) ||
      this.userValue['user_identifier'].length < 1
    ) {
      return [];
    }
    ...
    this.userValue['user_identifier'].forEach((identifier) => {
      if (
        ...
        matriculationNumber =
          identifier['value'] +
          ' / ' +
          Librarycardnumber.getDashedMatriculationNumber(identifier['value']);
      }
    });
  ...
  addLibraryCardNumber(
    ...
    libraryCardNumber =
      Librarycardnumber.sanitizeLibraryCardNumber(libraryCardNumber);
    // check if number exists already
    if (this.getLibraryCardNumbers().indexOf(libraryCardNumber) != -1)
      return false;
    ...
    if (
      Librarycardnumber.isValidImmatriculationNumber(libraryCardNumber, false)
    ) {
      // create matriculation number Object
      const immatriculationObject = {};
      const dashedMatriculationNumber =
        Librarycardnumber.getDashedMatriculationNumber(libraryCardNumber);
      const currentDate = new Date();
  ...
  removeLibraryCardNumber(libraryCardNumber: string): boolean {
    const initialCount = this.userValue['user_identifier'].length;
    const isImmatriculationNumber =
      Librarycardnumber.isValidImmatriculationNumber(
        libraryCardNumber['value'],
        false,
      );

    this.userValue['user_identifier'] = this.userValue[
      'user_identifier'
    ].filter(function (identifier) {
      return !isImmatriculationNumber
        ? identifier['value'] !== libraryCardNumber['value']
        : // also remove matriculation number;
          identifier['value'] !== libraryCardNumber['value'] &&
            Librarycardnumber.getDashedMatriculationNumber(
              libraryCardNumber['value'],
            ) !== identifier['value'];
    });
  ...
  setPreferredAddress(address: object, url: string): boolean {
    ...
    this.addSetting(
      'preferredPostalAddressType',
      address['address_type'][0]['value'],
      url,
    );
  ...
  getSettings(): [object, object] {
    ...
    if (
      !Array.isArray(this.userValue['user_note']) ||
      this.userValue['user_note'].length < 1
    ) {
    ...
    this.userValue['user_note'].forEach((setting) => {
      ...
      const regexMatches = setting['note_text'].match(/User Settings: (.+)/);
  ...
  addSetting(key: string, value: string, url: string) {
    ...
    const isUserSettingsExisting = settings[0] != null;

    settings[1][key] = value;

    const noteText = 'User Settings: ' + JSON.stringify(settings[1]);
